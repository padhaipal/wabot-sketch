...
The 10 second number will be adjusted based off of metrics to help meet the service level objective of having 99.XX% of messages responded to within 25 seconds over a 24 hour window. Every time we hit Redis, the database or an API call we will have this fallback exponential backoff system in place. We could set this retry window such that even if all of these backoffs succeed just before their capped times and still meet the 25 second limit. However, given how unlikely that is we can instead base the time cap on the metrics we observe will likely achieve the service level objective. This means we don't give up on retries prematurely.
