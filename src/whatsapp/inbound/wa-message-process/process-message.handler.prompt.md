...
3.) I will then detect consecutive messages from the user by checking if the inflight set exists for this user:
	  `EXISTS "{wabot:${ENV}}:inflight:<wa_id>"`
	  This returns 1 if inflight messages exist, 0 if not.
* If the call to Redis fails then log a WARN and do exponential backoff with a 10 second cap logging a WARN for every failed attempt.
  * If the backoff cap is reached then log/metric/trace metadata an ERROR. 
  * Run sendFallbackMessage (see send-fallback-message.prompt.md).
* If `EXISTS` returns 0 then PadhaiPal has responded to the last user's message that the WhatsApp bot has received. I need to add the wamid to the inflight set with `SADD "{wabot:${ENV}}:inflight:<wa_id>" "<wamid>"` and set `EXPIRE "{wabot:${ENV}}:inflight:<wa_id>" 25` (25s TTL) and then proceed as normal. 
