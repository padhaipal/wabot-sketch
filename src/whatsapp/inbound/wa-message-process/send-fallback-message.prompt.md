...
* If I get a 429/5XX response then log/metrics/span metadata a WARN 
  * If the messageTimestamp is more than 20s old. Mark the job as failed so BullMQ doesn't try again. Terminate the worker and span.
  * If the messageTimestampe is less than 20s old then do exponential backoff for up to 25s after messageTimestamp.
