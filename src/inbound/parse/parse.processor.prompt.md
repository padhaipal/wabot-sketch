...
* If `.changes[].value.messages` exists then check it's data shape against wabot/src/inbound/process/message/message.dto.ts and/or wabot/src/inbound/process/message/message.dto.prompt.md.
* If `.changes[].value.statuses` exists then check it's data shape against wabot/src/inbound/process/status/status.dto.ts and/or wabot/src/inbound/process/status/status.dto.prompt.md.
* If `.changes[].value.errors` exists then check it's data shape against wabot/src/inbound/process/error/error.dto.ts and/or wabot/src/inbound/process/error/error.dto.prompt.md.
