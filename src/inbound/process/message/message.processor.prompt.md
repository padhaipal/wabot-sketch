...
* Else continue.
4.) (This is completed asynchronously at the same time as step five) Run src/outbound/outbound.service.ts/sendReadAndTypingIndicator().
...
7.) outbound/outbound.service.ts/sendPPMessage() !!! I need to define data structure that includes consecutive flag !!!
...
* This is what the process-message-timeout job does when it is executed. This code is defined in this file.
  * it runs outbound/outbound.service.ts/sendWAMessage() with FALL_BACK_MESSAGE_EXTERNAL_ID


* Send fall back message. Call src/outbound/outbound.service.ts/sendWAMessage() with .env variabl FALL_BACK_MESSAGE_EXTERNAL_ID. Note that sendWAMessage() handles retries on 5XX. 
